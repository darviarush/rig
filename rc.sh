...
mr() {
    ...
    opera "$x?merge_request[assignee_ids][]=1036&merge_request[reviewer_ids][]=$GITLAB_REVIEWER&merge_request[source_branch]=`branch`&merge_request[target_branch]=$branch&merge_request[force_remove_source_branch]=1&merge_request[squash]=1&merge_request[title]=`branch`+`desc`" &> /dev/null
}
